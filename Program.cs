    ...
    internal class Program
    {
        public static void Main(string[] args)
        {
            try
            {
                ...
                Reigns[] ress = new Reigns[4];
                ReadFile("Cards.txt", res);
                ReadFile("Cardss.txt", ress);
                //Console.WriteLine("\n=== СОРТИРОВКА МАССИВА ===");
                //SortCards(res);
                ...
                // 1) найти пересечение одинаковых значений армии
                var q = (from card1 in res where card1 != null select card1.Army).Intersect(from card2 in ress where card2 != null select card2.Army);
                // 2) все карты с People = 20  упорядоченные по убыванию id
                var qq = (from f in res where f != null && f.People == 20 && (f is MagicCard || f is BattleMagicCard) orderby f.Id descending select f);
                // 3) только магические карты с Army < 20
                var qqq = (from f in res where f is MagicCard && f.Army < 20 select f);
                // 4) только карты королевства с Church > 20, но < 60 упорядоченные по 
                var qqqq = (from f in res where f != null && f is KingdomCard && f.Church > 20 && f.Church < 60 orderby f.Money select f);

                foreach (var qweqw in q) Console.WriteLine(qweqw); Console.WriteLine("+++++++++++++++++++++++");
                foreach (var qweqw in qq) Console.WriteLine(qweqw); Console.WriteLine("+++++++++++++++++++++++");
                Pr(qqq);
                Pr(qqqq);
            }
            catch (Exception ex)
        ...
        public static void Pr(IEnumerable<Reigns> q)
        {
